...
- **ETF 净申购/净赎回 TOP10 子块**（`collect_etf_flow.py`，复现 Wind ETF 净申赎榜，2026-08-31 新增）
  - 数据源：沪市 `ak.fund_etf_scale_sse(date)`（上交所，支持历史）；深市 `http://www.szse.cn/api/report/ShowReport`（**本机 https 被阻断，必须用 http**，CATALOGID=scsj_fund_jjgm，支持≤6个月区间）
  - 口径：本周净申赎＝当日份额−上周五份额（本周以来累计，周五即完整周度）；本年净申赎＝当日−2025-12-31；总份额＝当日值；单位亿份；按名称剔除货币型 ETF
  - 双榜：净申购 TOP10（红）/ 净赎回 TOP10（绿），列＝#/名称/代码/本周/本年/总份额；表 class=`etf-tb`（表头右留 158px 避让 cutoff-badge）
  - 快照库 `data/etf_share_snap.json` 长期累积；当日榜 `data/{date}_etf_flow.json`；历史趋势模块键 `etf_inflow`/`etf_outflow`（前端 `ind_etf_both` 双表，净赎回按 count 升序取最负在前）
  - **仅收盘版写历史趋势**（午间 `--midday` 传 `--no-hist`）；午间当日份额未公布时自动回退最近交易日并标 `is_t1`（标题显示⚠ T-1）
...
### 模块十：口袋 & 领先 & 断层
- **数据源**：通达信本地备份 `TdxBak_YYYYMMDD/T0002/blocknew/` 下三个自定义板块
  - `KD.blk` — 口袋支点个股
  - `2-LXG.blk` — 领先股
  - `2-JLRDC.blk` — 净利润断层
  - 三个板块合并去重，每只股票在 `source` 字段记录来源（口袋/领先股/断层，可多选，如「口袋+断层」）
- **更新脚本**：`update_koudai.py`（每个交易日16:50自动执行，午间版12:00执行）
- **展示**：分三个子表（口袋 / 领先股 / 净利润断层），均按一级行业排序；表头：序号 / 名称 / 代码 / 一级行业 / 二级行业 / 三级行业；行业分布统计按一级行业三表合并
- **数据日期**：从备份目录名提取（如 TdxBak_20260826 → date=2026-08-26），当天备份不存在时自动回退到最近8天内备份，页面显示红色感叹号
- **本地行业字典自动补全**（核心机制）：
  - 新增股票时，优先从以下本地数据自动补全名称和行业信息，无需调用问小达：
    1. `name_map.json` / `name_map_full.json` — 股票代码→名称映射
    2. `tdx_stock_industry.json` — 通达信研究行业三级分类（个股→一/二/三级行业，优先使用）
    3. `stock_classify.json` — 个股→行业/概念映射（industry 数组按从细到粗排列：[三级, 二级, 一级]，兜底）
    4. `tdx_industry_flat.json` / `tdx_industry_tree.json` — 通达信行业字典，补全一级行业
  - 本地未找到的股票才标记为"待查询"并发送飞书提醒，需手动补全
- **板块十一联动**：口袋+领先+断层个股在板块十一强势榜中每只记 2 分
- **历史趋势**：行业分布历史数据仅收盘版写入 `hist_industry_dist.json`，午间版不写入
...
| `collect_etf_flow.py` | 板块二 ETF 净申购/净赎回双榜：沪市sse+深市http日份额→快照累积→周/年净申赎→双榜+历史趋势（`--backfill N` 回填、`--midday` 不写历史） |
| `update_koudai.py` | 板块十数据自动更新：读取通达信备份 KD.blk（口袋）+2-LXG.blk（领先股）+2-JLRDC.blk（净利润断层）→ 合并去重、记录来源 → **优先从本地行业字典自动补全名称和行业信息** → 渲染+部署 |
...
| `koudai_industry.json` | 板块十股票列表+行业信息（口袋+领先股+净利润断层合并去重，source 记录来源，update_koudai.py 维护） |
| `{date}_etf_flow.json` | 板块二 ETF 净申赎双榜当日结果（inflow/outflow，含 week/year/total、week_base、year_base、data_date/is_t1） |
| `etf_share_snap.json` | 全市场 ETF 每日份额快照库（长期累积，collect_etf_flow.py 维护，周/年净申赎的对比基准） |
| `tdx_industry_tree.json` | 通达信行业字典树形结构（470节点：30一级/127二级/312三级，从 cloud_cfg/hy_tree.xml 解析） |
| `tdx_industry_flat.json` | 通达信行业字典扁平化（key=blockid，含parent指针，用于查找一级行业） |
| `hist_industry_dist.json` | 行业分布历史数据（6模块：limit_up/limit_down/top100/high_new/low_new/koudai，仅收盘版写入） |
