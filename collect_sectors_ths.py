...
import json, sys, os, time, argparse, datetime, re
...
def main():
    ...
    # 新增：领涨/领跌板块前15（按涨跌幅排序，过滤成交额不足40亿的板块，去重层级分类）
    pct_rows = [x for x in rows if x["pct"] is not None and x.get("turnover_yi", 0) >= 40]
    # 去重：根据名称基础部分（去掉Ⅱ、Ⅲ后缀）去重，保留成交额最大的
    def _base_name(name):
        return re.sub(r'[ⅡⅢ]+$', '', name)
    _seen = {}
    for r in pct_rows:
        bn = _base_name(r["name"])
        if bn not in _seen or r.get("turnover_yi", 0) > _seen[bn].get("turnover_yi", 0):
            _seen[bn] = r
    pct_rows = list(_seen.values())
    # 清理名称中的Ⅱ、Ⅲ后缀
    for r in pct_rows:
        r["name"] = _base_name(r["name"])
    pct_top = sorted(pct_rows, key=lambda x: x["pct"], reverse=True)[:15]  # 领涨
    pct_bot = sorted(pct_rows, key=lambda x: x["pct"])[:15]  # 领跌
    result = {
        "date": date,
        "source": SOURCE,
        "top_sectors": top_sectors,
        "net_inflow_sectors": {"top": net_top, "bottom": net_bot},
        "pct_sectors": {"top": pct_top, "bottom": pct_bot},  # 领涨/领跌板块
    }
