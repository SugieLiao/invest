...
def _resolve_node():
    """动态解析 node：workbuddy 的 node 版本目录会随升级改名（如 22.22.2 → 22.22.2-2），
    硬编码版本号会在升级后失效，故优先读 versions/current 标记文件，缺失则 glob 兜底。"""
    import glob
    vroot = "/Users/sugieliao/.workbuddy/binaries/node/versions"
    cur = os.path.join(vroot, "current")
    try:
        ver = open(cur, encoding="utf-8").read().strip()
        p = os.path.join(vroot, ver, "bin", "node")
        if os.path.exists(p):
            return p
    except Exception:
        pass
    cands = sorted(glob.glob(os.path.join(vroot, "*", "bin", "node")))
...
def stage_dyn(midday, date):
    ...
    写入 stock-a/dyn/（收盘）或 stock-a/dyn/noon/（午间）。
    ...
    target = os.path.join(dyn_root, "noon") if midday else dyn_root
...
def place_legacy_redirect():
    """旧中文「午盘」路径 → noon。Cloudflare Pages 的 _redirects 对中文 source
    匹配不稳定，改为在旧目录放一个即时跳转页（相对路径跳到 ../noon/）。"""
    page = ('<!doctype html><html lang="zh"><head><meta charset="utf-8">'
            '<meta http-equiv="refresh" content="0;url=../noon/">'
            '<script>try{location.replace("../noon/")}catch(e){}</script></head>'
            '<body style="font-family:sans-serif;color:#888;padding:40px">'
            '午间复盘已迁移至 <a href="../noon/">/stock-a/noon/</a></body></html>')
    for d in (os.path.join(PUB_STOCK, "午盘"), os.path.join(PUB_STOCK, "dyn", "午盘")):
        os.makedirs(d, exist_ok=True)
        with open(os.path.join(d, "index.html"), "w", encoding="utf-8") as f:
            f.write(page)
    print("  [deploy] 已放置旧「午盘」路径 → noon 跳转页", flush=True)
...
def main():
    ...
    if midday:
        print("  [mode] 午间版：渲染 A股午盘_{date}.html，部署到 stock-a/noon/", flush=True)
    ...
    # 均线占比（均占系统）：午间版/收盘版都尝试刷新；token 过期或接口失败时
    # collect_jzxt.py 保留上一交易日数据不覆盖（页面继续显示旧数据，不显示"暂不可达"）
    run([SYS_PY, "collect_jzxt.py"], "jzxt(均线占比)")
    # TR情绪监测（通达信扩展数据 38/39/40）：从 Parallels VM 复制扩展数据文件并解析
    run([SYS_PY, "collect_tr_emotion.py"], "tr_emotion(通达信扩展数据·TR情绪监测)")
    # 沪深两市融资余额+融资买入额（akshare批量接口，全量快速更新）
    run([VENV_PY, "collect_margin.py", "--increment"], "margin(融资余额/融资买入额)")
    # ETF 净申购/净赎回双榜（上交所sse + 深交所http日频份额；午间不写历史趋势）
    etf_cmd = [VENV_PY, "collect_etf_flow.py", date]
    if midday:
        etf_cmd.append("--midday")
    run(etf_cmd, "etf_flow(ETF净申购/净赎回双榜·板块二)")
    # 板块十二 主线回踩买点（仅概念板块，pytdx 实时算5/20日涨幅；收盘/午间都跑，午间反映上午）
    mainline_cmd = [VENV_PY, "collect_mainline.py", date]
    if midday:
        mainline_cmd.append("--midday")
    run(mainline_cmd, "mainline(主线回踩买点·概念板块·板块十二)")
    # 全市场成交额长期历史（东方财富指数合成，补全hithink之前的日期，供成交额曲线/两融占比）
    run([SYS_PY, "collect_turnover_history.py", "2"], "turnover_history(全市场成交额长期历史)")
    ...
    # 目标子目录：午间版 → stock-a/noon/；收盘版 → stock-a/（站点根）
    sub = "noon" if midday else ""
    out_dir = os.path.join(PUB_STOCK, sub) if sub else PUB_STOCK
    os.makedirs(out_dir, exist_ok=True)
    for f in ("CNAME", ".nojekyll", "_redirects"):
        src = os.path.join(BASE, f)
        if os.path.exists(src):
            shutil.copy(src, os.path.join(PUB, f))
    place_legacy_redirect()
    ...
    if sub:
        ...
        print(f"  [deploy] 午间版 → stock-a/noon/ （{os.path.basename(html)} + index.html）", flush=True)
    else:
