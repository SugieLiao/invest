...
数据来源（2026-09-05 起：主源板块池由旧 881 行业改为 880 概念，与板块十二同源）：
    主源 1：通达信 TDX（pytdx 直连公共行情服务器，无鉴权）
            概念板块指数 880xxx 的日K（get_index_bars，命令 0x8c，前复权）。
            板块代码与名称取自本地通达信备份 tdxzs3.cfg 中 880 类型=4 的概念板块
            （复用 collect_mainline.find_cfg/load_concepts，剔风格属性后约261个）；
            880 概念在 get_security_list 中枚举不到名称，故不再用服务器名称表。
    ...
    历史：旧 881 行业口径的枚举/名称函数（tdx_security_names、TDX_CONCEPT_START/END）
            保留给 backfill_sector_rps.py 回算历史，主流程不再使用。

板块范围（默认概念板块，让市场自己走出来；不预设名单）：
    RPS_UNIVERSE=concept   仅概念板块（本地 tdxzs3.cfg 880 类型=4 清单，与板块十二同源）—— 默认
    RPS_UNIVERSE=watchlist 自选板块：data/sector_watchlist.txt（每行一个板块名，如「半导体」）
                           —— 仅当你自己有固定观察池时再用，默认不用
    （TDX 主源板块池＝880 概念；旧 881 行业枚举仅 backfill 历史使用）
...
# ---- 双源容错：通达信 TDX(主, 概念板块指数 880xxx·本地概念清单日K) + 同花顺 THS(备, 官网概念板块 line 接口) ----
...
TDX_MARKET = 1                 # 概念板块指数(880xxx) 在 TDX 的 market
TDX_CONCEPT_START = 881000     # 【旧 881 行业口径】仅 backfill_sector_rps.py 回算历史时使用
TDX_CONCEPT_END = 882000       # 主路径 collect_via_tdx 已改用 880 本地概念清单（tdxzs3.cfg 类型4）
...
def collect_via_tdx(target_date):
    """按本地概念清单枚举通达信概念板块指数(880xxx)，批量取日K，算收益。返回 all_rets 或 None。

    板块池＝tdxzs3.cfg 中 880 类型=4 的概念板块（与板块十二 collect_mainline 严格同源，
    复用其 find_cfg/load_concepts，剔风格属性后约 261 个）；板块名直接来自本地清单。

    关键坑1：概念板块指数是『指数』，必须用 get_index_bars（命令 0x8c）取日K；
    用 get_security_bars（命令 0x81，股票口径）会返错乱垃圾（负收盘、年份 29xxxx 等）。
    关键坑2：880 概念在 pytdx get_security_list 里枚举不到名称，故代码/名称一律以本地
    tdxzs3.cfg 类型=4 为准，不再走 tdx_security_names（那是旧 881 行业口径用的）。
    """
    try:
        from collect_mainline import find_cfg, load_concepts
    except Exception as e:
        print(f"[sector_rps] 无法导入概念清单模块 collect_mainline：{e}")
        return None
    cfg_path, cfg_date, fb_days = find_cfg(target_date)
    if not cfg_path:
        print(f"[sector_rps] 最近8天通达信备份均无 tdxzs3.cfg（目标日 {target_date}），TDX 源放弃")
        return None
    concept_map, raw_n = load_concepts(cfg_path)
    if not concept_map:
        print(f"[sector_rps] 概念清单为空：{cfg_path}")
        return None
    fb = f"，回退{fb_days}天" if fb_days else ""
    print(f"[sector_rps] 概念清单日期 {cfg_date}{fb}：类型4原始 {raw_n}，剔风格后 {len(concept_map)} 个")
    ...
    try:
        for i, (c, name) in enumerate(items, 1):
            try:
            ...
            if i % 80 == 0:
                print(f"  ...概念取K {i}/{len(items)}，累计有效 {len(boards)}")
    finally:
        try:
            api.disconnect()
        except Exception:
            pass
    print(f"[sector_rps] TDX 概念取K完成：有效概念板块 {len(boards)} 个（清单 {len(concept_map)}）")
    if not boards:
        return None
    # 流动性预筛选（按成交额）
    if PRE_FILTER:
...
def main():
    source = RPS_SOURCE
    # 可选命令行参数 YYYY-MM-DD：用于补算指定日（找概念清单以该日为准回退8天）；默认运行当天
    ...
    # ---- 源1：通达信 TDX（主源，概念板块指数 880xxx，本地 tdxzs3.cfg 类型4 清单，与板块十二同源）----
    if source in ("auto", "tdx"):
        print("[sector_rps] 源=通达信 TDX：按本地概念清单取 880 概念板块日K ...")
        all_rets = collect_via_tdx(target_date)
        if all_rets:
            src_str = "通达信 TDX（概念板块指数 880xxx·本地概念清单，前复权；主源）"
            total_boards = len(all_rets)   # 已含流动性预筛
            save_name_cache(all_rets)      # 写名称缓存，供后续复用
    ...
    # 每日快照：供 render.py 聚合 RPS 历史趋势表（模块五）
    if latest:
        snap = os.path.join(BASE, "data", f"{latest}_sector_rps.json")
