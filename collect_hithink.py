...
# 顺序即页面展示顺序：全A → 创历史新高 → 近期创一年新高 → 昨日成交前10 → 微盘股 → 北证50 → 昨日涨停
STYLE_INDICES = [
    ("883957.TI", "全A（沪深京）", "hithink"),
    ("883911.TI", "创历史新高", "hithink"),
    ("883925.TI", "近期创一年新高", "hithink"),
...
def fetch_total_turnover():
    """获取全市场成交额（亿）：上证综指 + 深证综指 + 北证50。
    主：腾讯行情 qt.gtimg.cn；备：东财 push2 ulist。
    比 hithink 个股求和更准确（hithink 快照可能缺部分股票）。
    """
    ...
    if turn is None:
        try:
            out = subprocess.run(["curl", "-s", "--max-time", "15",
                                  ...
                                  "https://push2.eastmoney.com/api/qt/ulist.np/get?fltt=2&secids=1.000001,0.399106,0.899050&fields=f6"],
...
def collect_market(nm, date):
    ...
    # 用指数成交额（上证+深证综指+北证50）替换个股求和，更准确
    idx_turn = fetch_total_turnover()
    if idx_turn is not None:
        total_turn_yi = idx_turn
...
def collect_indices():
    ...
    start = end - 800 * 86400 * 1000  # ~560自然日≈500交易日（两年）
    out = []
    for code, name in INDICES:
        ...
        if len(dates) > 500:  # 裁到最近 500 交易日
            dates, closes = dates[-500:], closes[-500:]
        out.append({
            "code": code, "name": name,
...
def main():
    ...
    result["collected_at"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
