...
def _resolve_node():
    """动态解析 node：版本目录随升级改名，优先读 versions/current，缺失则 glob 兜底。"""
    import glob
...
def main():
    ...
    # 0. 站点级文件（CNAME / .nojekyll / 旧午盘路径 301 → noon）
    ...
    out_mid = os.path.join(PUB_STOCK, "noon")
    ...
    if os.path.exists(mid_html):
        ...
        print(f"[deploy] 午间版 → noon/ (index.html = {DATE})", flush=True)
